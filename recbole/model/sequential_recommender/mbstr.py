...
from recbole.model.new_transformer import TransformerBlock
from recbole.model.heads import CGCDotProductPredictionHead

class MBSTR(SequentialRecommender):

    def __init__(self, config, dataset):
        super(MBSTR, self).__init__(config, dataset)

        # load parameters info
        self.n_layers = config['n_layers']
        self.n_heads = config['n_heads']
        self.n_behavior = config['n_behavior']
        ...
        self.battn = config['battn']
        self.bpff = config['bpff']
        self.brpb = config['brpb']
        ...
        self.multi_behavior_transformer = nn.ModuleList([TransformerBlock(self.hidden_size, self.n_heads,
                                                                          self.hidden_size * 4, self.n_behavior,
                                                                          self.battn, self.bpff, self.brpb,
                                                                          self.attn_dropout_prob) for _ in range(self.n_layers)])
        self.head = CGCDotProductPredictionHead(self.hidden_size, self.n_behavior, 3, 1, self.n_items, self.item_embedding)
    ...
    def forward(self, item_seq, type_seq, mask_positions_nums=None, session_id=None):
        ...
        for transformer in self.multi_behavior_transformer:
            x = transformer.forward(x, type_seq, mask)
        x = self.head.forward(x, type_seq)
        return x  # [B L H]
